...
let createEntryAdmin = express.Router();
const leapYear = (year) => {
...
createEntryAdmin.post('/', async (req, res) => {

    try {
        //The time is taken in 24 hour clock
        let { uuidAdmin, uuidUser, dayTaken, monthTaken, yearTaken, hourTaken, minuteTaken, product, calorie } = req.body;
        ...
        //checking if minute is correct
            ...
            return res.status(200).json({
                messsage: `Entry for ${product} Created for user ${UserFind.fname}`
            });
        }
...
export default createEntryAdmin;
